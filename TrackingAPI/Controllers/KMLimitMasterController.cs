...
namespace TrackingAPI.Controllers
{
    ...
    [ApiController]
    public class KMLimitMasterController : ControllerBase
    {
        private readonly IConfiguration _configuration;

        public KMLimitMasterController(IConfiguration configuration)
        {
            _configuration = configuration;
        }

        [HttpGet]
        [Route("KMLimitMaster_SelectAll")]
        public JsonResult KMLimitMaster_SelectAll()
        {
            string query = "DM_sp_KMLimitMaster_SelectAll";
        ...
        [HttpGet]
        [Route("KMLimitMaster_SelectAll_Active")]
        public JsonResult KMLimitMaster_SelectAll_Active()
        {
            string query = "DM_sp_KMLimitMaster_SelectAll_Active";
            DataTable table = new DataTable(); string sqlDataSource = _configuration.GetConnectionString("EmployeeAppCon"); SqlDataReader myReader;
            using (SqlConnection myCon = new SqlConnection(sqlDataSource))
            {
                myCon.Open(); using (SqlCommand myCommand = new SqlCommand(query, myCon))
        ...
        [HttpPost]
        public JsonResult Post(KMLimitMaster CM)
        {
            try
            {
                string query = "DM_sp_KMLimitMaster_Insert";
                ...
                using (SqlConnection myCon = new SqlConnection(sqlDataSource))
                {
                    myCon.Open(); using (SqlCommand myCommand = new SqlCommand(query, myCon))
                    {
                        ...
                        myCommand.Parameters.AddWithValue("nKMLimit", CM.nKMLimit);
        ...
        [HttpPut]
        public JsonResult Put(KMLimitMaster CM)
        {
            try
            {
                string query = "DM_sp_KMLimitMaster_Update";
                DataTable table = new DataTable(); string sqlDataSource = _configuration.GetConnectionString("EmployeeAppCon"); int retValue = 0;
                ...
                using (SqlConnection myCon = new SqlConnection(sqlDataSource))
                {
                    ...
                    {
                        myCommand.CommandType = CommandType.StoredProcedure;
                        myCommand.Parameters.AddWithValue("nLimitId", CM.nLimitId);
                        myCommand.Parameters.AddWithValue("nCityId", CM.nCityId);
                        myCommand.Parameters.AddWithValue("nVId", CM.nVId);
                        myCommand.Parameters.AddWithValue("nKMLimit", CM.nKMLimit);
